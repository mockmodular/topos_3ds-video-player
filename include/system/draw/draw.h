 ...
 * @param _3d (in) When true, enable stereoscopic 3D on the top screen (mutually exclusive with 2D in hardware).
 ...
 * @param _3d (in) When true, enable stereoscopic 3D on the top screen.
 * @return On success DEF_SUCCESS, on failure DEF_ERR_*.
 * @warning Thread dangerous (untested).
 * @warning Call it only from rendering thread.
*/
uint32_t Draw_reinit(bool _3d);
...
/**
 * @brief Ready screen and fill it with abgr8888.
 * Do nothing if draw API is not initialized.
