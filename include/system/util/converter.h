...
/**
 * @brief Convert YUV420P to RGBA8888 writing directly into a pre-allocated texture buffer.
 * Y2R hardware uses transfer_gap so each tile-row lands at the correct tex_width stride.
 * No linearAlloc/free performed; caller must provide a valid linear VRAM buffer.
 * @param yuv420p  (in) YUV420P input frame (width x height).
 * @param dst      (in) Destination texture data pointer (linear VRAM, tex_width x height * 4 bytes).
 * @param tex_width (in) Texture stride width in pixels (next power of 2 >= width).
 * @param width    (in) Frame pixel width (must be multiple of 2).
 * @param height   (in) Frame pixel height (must be multiple of 2).
 * @return On success DEF_SUCCESS, on failure DEF_ERR_* or Nintendo API's error.
 * @warning Thread dangerous (untested). Requires util_y2r_init.
*/
...
/**
 * @brief Convert SBS YUV420P directly to two RGBA8888 eye buffers via two Y2R passes with stride gap.
 * Eliminates tile-row splitting; Y2R hardware skips the opposite eye's data via gap parameter.
 * Both output buffers are allocated internally with linearAlloc (BLOCK_8_BY_8 pre-tiled).
 * @param yuv420p (in) Full-width SBS YUV420P frame (full_width x height).
 * @param left_rgba (out) Left eye output; reallocated to (full_width/2 * height * 4) bytes.
 * @param right_rgba (out) Right eye output; reallocated to (full_width/2 * height * 4) bytes.
 * @param full_width (in) Full SBS frame width (must be multiple of 16).
 * @param height (in) Frame height (must be multiple of 8).
 * @return On success DEF_SUCCESS, on failure DEF_ERR_* or Nintendo API's error.
 * @warning Thread dangerous (untested). Requires util_y2r_init.
*/
...
#define Util_converter_y2r_yuv420p_to_rgba8888(...) DEF_ERR_DISABLED
#define Util_converter_y2r_yuv420p_to_rgba8888_direct(...) DEF_ERR_DISABLED
#define Util_converter_y2r_yuv420p_sbs_to_rgba8888(...) DEF_ERR_DISABLED
#define Util_converter_y2r_exit()

#endif //DEF_CONVERTER_HW_API_ENABLE

/**
 * @brief Free cached FFmpeg SwsContext and SwrContext.
 * Call this when playback stops to release resources.
 * @note Thread safe.
*/
