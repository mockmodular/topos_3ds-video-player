...
/** Bitfield for entries returned by Util_file_read_dir(). */
typedef uint8_t Util_file_dir_type;
#define UTIL_FILE_DIR_TYPE_NONE      (Util_file_dir_type)(0)
#define UTIL_FILE_DIR_TYPE_FILE      (Util_file_dir_type)(1 << 0)
#define UTIL_FILE_DIR_TYPE_DIR       (Util_file_dir_type)(1 << 1)
#define UTIL_FILE_DIR_TYPE_READ_ONLY (Util_file_dir_type)(1 << 2)
#define UTIL_FILE_DIR_TYPE_HIDDEN    (Util_file_dir_type)(1 << 3)
 ...
 * @param read_size (out) Actual read size (in byte).
 * @return On success DEF_SUCCESS, on failure DEF_ERR_* or Nintendo API's error.
 * @note Thread safe.
*/
 ...
 * @note Thread safe.
*/
uint32_t Util_file_read_dir(const char* dir_path, uint32_t* detected, Str_data* file_name, Util_file_dir_type* type, uint32_t array_length);
