...
void Vid_convert_thread(void* arg)
{
	...
	while (vid_player.thread_run)
	{
		...
		if(vid_player.num_of_audio_tracks > 0 && vid_player.audio_info[0].sample_rate > 0)
		{
			uint8_t playing_ch = (vid_player.audio_info[0].ch > 2 ? 2 : vid_player.audio_info[0].ch);
			...
			C3D_TexFlush(di_mvd->c2d.tex);
				...
				if(result == DEF_SUCCESS)
				{
					...
					if(vid_player.num_of_audio_tracks == 0)
					{
						...
						total_delay += osTickCounterRead(&conversion_time_counter) - vid_player.video_frametime[packet_index];
					}
