...
void Util_fake_pthread_set_enabled_core(const bool enabled_core[4], int start_core)
{
	int num_of_core = 0;
	int start_offset = 0;
	...
	// Collect in 0→2→1→3 order so N3DS (C0,C1,C2) round-robin is C0→C2→C1 (…021021…).
	// O3DS {C0,C1} becomes [0,1]; start_core=1 → offset 1 → C1→C0→….
	static const int collect_order[4] = { 0, 2, 1, 3, };

	for(int j = 0; j < 4; j++)
	{
		int i = collect_order[j];
		if(enabled_core[i])
			util_fake_pthread_enabled_core_list[num_of_core++] = i;
	}

	//Start workers from the first core in the list whose number >= start_core.
	//N3DS: start_core=0 → list [0,2,1] from C0; O3DS: start_core=1 → [0,1] from C1.
	start_offset = 0;
	for(int i = 0; i < num_of_core; i++)
	{
		if(util_fake_pthread_enabled_core_list[i] >= start_core)
		{
			start_offset = i;
			break;
		}
	...
	util_fake_pthread_core_offset = start_offset;
}
...
#pragma GCC diagnostic pop
	...
	if(__attr)
		handle = threadCreate(entry_point, __arg, __attr->stacksize, DEF_THREAD_PRIORITY_NORMAL, util_fake_pthread_enabled_core_list[util_fake_pthread_core_offset], true);
	else
		handle = threadCreate(entry_point, __arg, DEF_THREAD_STACKSIZE, DEF_THREAD_PRIORITY_NORMAL, util_fake_pthread_enabled_core_list[util_fake_pthread_core_offset], true);
