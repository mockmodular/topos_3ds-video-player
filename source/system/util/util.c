...
//Heap low cache: only probe heap every 100ms instead of every malloc call.
#define HEAP_LOW_CACHE_INTERVAL_MS 100
static bool cached_is_heap_low = false;
static uint64_t cached_heap_check_time = 0;
...
static inline bool Util_is_heap_low(void)
{
	...
	if((now - cached_heap_check_time) < HEAP_LOW_CACHE_INTERVAL_MS)
		return cached_is_heap_low;

	//Time to re-probe the heap.
	bool is_low = true;
	void* ptr = NULL;
	...
	//Update cache.
	cached_is_heap_low = is_low;
	cached_heap_check_time = now;
