...
uint32_t Util_file_read_dir(const char* dir_path, uint32_t* detected, Str_data* file_name, Util_file_dir_type* type, uint32_t array_length)
{
	...
	for(uint32_t i = 0; i < array_length; i++)
	{
		...
		type[i] = UTIL_FILE_DIR_TYPE_NONE;
	}
	*detected = 0;
	...
	while (true)
	{
		...
		if (fs_entry.attributes & FS_ATTRIBUTE_HIDDEN)
			type[count] = (type[count] | UTIL_FILE_DIR_TYPE_HIDDEN);
		if (fs_entry.attributes & FS_ATTRIBUTE_DIRECTORY)
			type[count] = (type[count] | UTIL_FILE_DIR_TYPE_DIR);
		if (fs_entry.attributes & FS_ATTRIBUTE_ARCHIVE)
			type[count] = (type[count] | UTIL_FILE_DIR_TYPE_FILE);
		if (fs_entry.attributes & FS_ATTRIBUTE_READ_ONLY)
			type[count] = (type[count] | UTIL_FILE_DIR_TYPE_READ_ONLY);
