...
//Cached SwsContext: avoids per-frame recreation during video playback.
#if DEF_CONVERTER_SW_FFMPEG_COLOR_API_ENABLE
static struct SwsContext* cached_sws_context = NULL;
static uint32_t cached_sws_in_width = 0;
static uint32_t cached_sws_in_height = 0;
static uint32_t cached_sws_out_width = 0;
static uint32_t cached_sws_out_height = 0;
static Raw_pixel cached_sws_in_fmt = RAW_PIXEL_INVALID;
static Raw_pixel cached_sws_out_fmt = RAW_PIXEL_INVALID;
#endif //DEF_CONVERTER_SW_FFMPEG_COLOR_API_ENABLE

//Cached SwrContext: avoids per-frame recreation during audio playback.
#if DEF_CONVERTER_SW_FFMPEG_AUDIO_API_ENABLE
static SwrContext* cached_swr_context = NULL;
static uint8_t cached_swr_in_ch = 0;
static uint32_t cached_swr_in_sample_rate = 0;
static uint8_t cached_swr_out_ch = 0;
static uint32_t cached_swr_out_sample_rate = 0;
static Raw_sample cached_swr_in_fmt = RAW_SAMPLE_INVALID;
static Raw_sample cached_swr_out_fmt = RAW_SAMPLE_INVALID;
#endif //DEF_CONVERTER_SW_FFMPEG_AUDIO_API_ENABLE
...
uint32_t Util_converter_convert_color(Converter_color_parameters* parameters)
{
	...
	//Reuse cached SwsContext if parameters match, otherwise recreate.
	if(!cached_sws_context || cached_sws_in_width != parameters->in_width || cached_sws_in_height != parameters->in_height
	|| cached_sws_out_width != parameters->out_width || cached_sws_out_height != parameters->out_height
	|| cached_sws_in_fmt != parameters->in_color_format || cached_sws_out_fmt != parameters->out_color_format)
	{
		sws_freeContext(cached_sws_context);
		cached_sws_context = sws_getContext(parameters->in_width, parameters->in_height, util_converter_pixel_format_table[parameters->in_color_format],
		parameters->out_width, parameters->out_height, util_converter_pixel_format_table[parameters->out_color_format], 0, 0, 0, 0);
		if(!cached_sws_context)
		{
			DEF_LOG_RESULT(sws_getContext, false, DEF_ERR_FFMPEG_RETURNED_NOT_SUCCESS);
			goto ffmpeg_api_failed;
		}
		cached_sws_in_width = parameters->in_width;
		cached_sws_in_height = parameters->in_height;
		cached_sws_out_width = parameters->out_width;
		cached_sws_out_height = parameters->out_height;
		cached_sws_in_fmt = parameters->in_color_format;
		cached_sws_out_fmt = parameters->out_color_format;
	}
	...
	ffmpeg_result = sws_scale(cached_sws_context, (const uint8_t* const*)src_data, src_line_size, 0, parameters->in_height, dst_data, dst_line_size);
	if(ffmpeg_result < 0)
	{
...
uint32_t Util_converter_convert_audio(Converter_audio_parameters* parameters)
{
	...
	//Reuse cached SwrContext if parameters match, otherwise recreate.
	if(!cached_swr_context || cached_swr_in_ch != parameters->in_ch || cached_swr_in_sample_rate != parameters->in_sample_rate
	|| cached_swr_out_ch != parameters->out_ch || cached_swr_out_sample_rate != parameters->out_sample_rate
	|| cached_swr_in_fmt != parameters->in_sample_format || cached_swr_out_fmt != parameters->out_sample_format)
	{
		AVChannelLayout out_ch_layout = AV_CHANNEL_LAYOUT_MONO;
		AVChannelLayout in_ch_layout = AV_CHANNEL_LAYOUT_MONO;

		swr_free(&cached_swr_context);

		av_channel_layout_default(&out_ch_layout, parameters->out_ch);
		av_channel_layout_default(&in_ch_layout, parameters->in_ch);

		ffmpeg_result = swr_alloc_set_opts2(&cached_swr_context, &out_ch_layout, util_converter_sample_format_table[parameters->out_sample_format], parameters->out_sample_rate,
		...
		ffmpeg_result = swr_init(cached_swr_context);
		if(ffmpeg_result != 0)
		{
		...
		cached_swr_in_ch = parameters->in_ch;
		cached_swr_in_sample_rate = parameters->in_sample_rate;
		cached_swr_out_ch = parameters->out_ch;
		cached_swr_out_sample_rate = parameters->out_sample_rate;
		cached_swr_in_fmt = parameters->in_sample_format;
		cached_swr_out_fmt = parameters->out_sample_format;
	}
	...
	ffmpeg_result = swr_convert(cached_swr_context, dst_data, parameters->out_samples, (const uint8_t* const*)src_data, parameters->in_samples);
	if(ffmpeg_result <= 0)
	{
...
uint32_t Util_converter_y2r_yuv420p_to_rgba8888(const uint8_t* yuv420p, uint8_t** rgba8888, uint16_t width, uint16_t height, bool texture_format)
{
	uint32_t result = DEF_ERR_OTHER;
	Y2RU_ConversionParams y2r_parameters;
	Handle conversion_finish_event_handle = 0;
	...
	if(!yuv420p || !rgba8888 || width == 0 || height == 0 || width % 2 != 0 || height % 2 != 0)
		goto invalid_arg;

	free(*rgba8888);
	*rgba8888 = (uint8_t*)linearAlloc(width * height * 4);
	if(!*rgba8888)
		goto out_of_memory;

	memset(&y2r_parameters, 0x0, sizeof(y2r_parameters));
	y2r_parameters.input_format = INPUT_YUV420_INDIV_8;
	y2r_parameters.output_format = OUTPUT_RGB_32;
	y2r_parameters.rotation = ROTATION_NONE;
	y2r_parameters.block_alignment = texture_format ? BLOCK_8_BY_8 : BLOCK_LINE;
	y2r_parameters.input_line_width = width;
	y2r_parameters.input_lines = height;
	...
	result = Y2RU_SetReceiving(*rgba8888, width * height * 4, width * 4 * 4, 0);
	if(result != DEF_SUCCESS)
	{
		DEF_LOG_RESULT(Y2RU_SetReceiving, false, result);
		goto nintendo_api_failed;
...
uint32_t Util_converter_y2r_yuv420p_to_rgba8888_direct(const uint8_t* yuv420p, uint8_t* dst, uint16_t tex_width, uint16_t width, uint16_t height)
{
	uint32_t result = DEF_ERR_OTHER;
	Y2RU_ConversionParams y2r_parameters;
	Handle conversion_finish_event_handle = 0;
	...
	result = Y2RU_SetConversionParams(&y2r_parameters);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetConversionParams, false, result); goto nintendo_api_failed; }

	result = Y2RU_SetSendingY(yuv420p, width * height, width, 0);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetSendingY, false, result); goto nintendo_api_failed; }

	result = Y2RU_SetSendingU(yuv420p + (width * height), width * height / 4, width / 2, 0);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetSendingU, false, result); goto nintendo_api_failed; }

	result = Y2RU_SetSendingV(yuv420p + (width * height) + (width * height / 4), width * height / 4, width / 2, 0);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetSendingV, false, result); goto nintendo_api_failed; }

	// transfer_unit = one full tile-row (width * 4 bytes/pixel * 8 rows)
	// transfer_gap  = padding to match tex_width stride
	result = Y2RU_SetReceiving(dst, (uint32_t)width * height * 4, (uint32_t)width * 4 * 8, (uint32_t)(tex_width - width) * 4 * 8);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetReceiving, false, result); goto nintendo_api_failed; }

	result = Y2RU_StartConversion();
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_StartConversion, false, result); goto nintendo_api_failed; }

	result = Y2RU_GetTransferEndEvent(&conversion_finish_event_handle);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_GetTransferEndEvent, false, result); goto nintendo_api_failed; }

	svcWaitSynchronization(conversion_finish_event_handle, 200000000);
	svcCloseHandle(conversion_finish_event_handle);
	return DEF_SUCCESS;
	...
	nintendo_api_failed:
	if(conversion_finish_event_handle)
		svcCloseHandle(conversion_finish_event_handle);
	return result;
}

uint32_t Util_converter_y2r_yuv420p_sbs_to_rgba8888(const uint8_t* yuv420p, uint8_t* dst_left, uint8_t* dst_right, uint16_t tex_width, uint16_t full_width, uint16_t height)
{
	uint32_t result = DEF_ERR_OTHER;
	Y2RU_ConversionParams y2r_parameters;
	Handle conversion_finish_event_handle = 0;
	uint16_t half_w = full_width / 2;
	uint32_t y_size = (uint32_t)full_width * height;          // Y plane bytes (full width)
	uint32_t uv_size = y_size / 4;                            // U or V plane bytes (full width)
	uint16_t half_uv_w = half_w / 2;                         // U/V half-eye pixel width

	if(!util_y2r_init)
		goto not_inited;

	if(!yuv420p || !dst_left || !dst_right || tex_width < half_w || full_width == 0 || height == 0
		|| full_width % 16 != 0 || height % 8 != 0)
		goto invalid_arg;
	...
	y2r_parameters.rotation = ROTATION_NONE;
	y2r_parameters.block_alignment = BLOCK_8_BY_8;
	y2r_parameters.input_line_width = half_w;   // half-eye width, not full_width
	y2r_parameters.input_lines = height;
	y2r_parameters.standard_coefficient = COEFFICIENT_ITU_R_BT_709_SCALING;
	y2r_parameters.alpha = 0xFF;

	// === Pass 1: left eye (starts at column 0, skip right half via gap) ===
	result = Y2RU_SetConversionParams(&y2r_parameters);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetConversionParams, false, result); goto nintendo_api_failed; }

	// Y: read half_w bytes, skip half_w bytes per row
	result = Y2RU_SetSendingY(yuv420p, (uint32_t)half_w * height, half_w, half_w);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetSendingY, false, result); goto nintendo_api_failed; }

	// U: read half_uv_w bytes, skip half_uv_w bytes per row
	result = Y2RU_SetSendingU(yuv420p + y_size, (uint32_t)half_uv_w * (height / 2), half_uv_w, half_uv_w);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetSendingU, false, result); goto nintendo_api_failed; }

	// V: same layout as U
	result = Y2RU_SetSendingV(yuv420p + y_size + uv_size, (uint32_t)half_uv_w * (height / 2), half_uv_w, half_uv_w);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetSendingV, false, result); goto nintendo_api_failed; }

	// transfer_unit = one full tile-row (half_w * 8 rows * 4 bytes/pixel)
	// transfer_gap  = padding to match tex_width stride ((tex_width - half_w) * 32 bytes)
	// Y2R writes directly into dst_left (l_tex->data) with correct texture stride; no CPU copy needed.
	result = Y2RU_SetReceiving(dst_left, (uint32_t)half_w * height * 4, (uint32_t)half_w * 4 * 8, (uint32_t)(tex_width - half_w) * 4 * 8);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetReceiving, false, result); goto nintendo_api_failed; }
	...
	svcWaitSynchronization(conversion_finish_event_handle, 200000000);
	svcCloseHandle(conversion_finish_event_handle);
	conversion_finish_event_handle = 0;

	// === Pass 2: right eye (starts at column half_w, same gap) ===
	result = Y2RU_SetConversionParams(&y2r_parameters);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetConversionParams, false, result); goto nintendo_api_failed; }

	// Y: right eye starts half_w bytes into Y plane
	result = Y2RU_SetSendingY(yuv420p + half_w, (uint32_t)half_w * height, half_w, half_w);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetSendingY, false, result); goto nintendo_api_failed; }

	// U: right eye starts half_uv_w bytes into U plane
	result = Y2RU_SetSendingU(yuv420p + y_size + half_uv_w, (uint32_t)half_uv_w * (height / 2), half_uv_w, half_uv_w);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetSendingU, false, result); goto nintendo_api_failed; }

	// V: right eye starts half_uv_w bytes into V plane
	result = Y2RU_SetSendingV(yuv420p + y_size + uv_size + half_uv_w, (uint32_t)half_uv_w * (height / 2), half_uv_w, half_uv_w);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetSendingV, false, result); goto nintendo_api_failed; }

	result = Y2RU_SetReceiving(dst_right, (uint32_t)half_w * height * 4, (uint32_t)half_w * 4 * 8, (uint32_t)(tex_width - half_w) * 4 * 8);
	if(result != DEF_SUCCESS) { DEF_LOG_RESULT(Y2RU_SetReceiving, false, result); goto nintendo_api_failed; }
...
void Util_converter_ffmpeg_cache_clear(void)
{
	#if DEF_CONVERTER_SW_FFMPEG_COLOR_API_ENABLE
	sws_freeContext(cached_sws_context);
	cached_sws_context = NULL;
	cached_sws_in_width = 0;
	cached_sws_in_height = 0;
	cached_sws_out_width = 0;
	cached_sws_out_height = 0;
	cached_sws_in_fmt = RAW_PIXEL_INVALID;
	cached_sws_out_fmt = RAW_PIXEL_INVALID;
	#endif //DEF_CONVERTER_SW_FFMPEG_COLOR_API_ENABLE

	#if DEF_CONVERTER_SW_FFMPEG_AUDIO_API_ENABLE
	swr_free(&cached_swr_context);
	cached_swr_in_ch = 0;
	cached_swr_in_sample_rate = 0;
	cached_swr_out_ch = 0;
	cached_swr_out_sample_rate = 0;
	cached_swr_in_fmt = RAW_SAMPLE_INVALID;
	cached_swr_out_fmt = RAW_SAMPLE_INVALID;
	#endif //DEF_CONVERTER_SW_FFMPEG_AUDIO_API_ENABLE
}
