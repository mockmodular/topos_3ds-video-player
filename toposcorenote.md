...
# topos_0.1 — Agent 会话架构同步（来源：`agent-transcripts`，2026-04）

> 以下为当前仓库 **topos_0.1** 相关会话中已落地的改动与结论摘要（非逐字聊天）。UUID：`18eefad5…` `e571d1f7…` `15333a78…` `3b8ce98c…`（`bed143ec…` 仅为元对话，无代码变更）。

## 版本号、CIA 元数据与 Git（0.12）

- **单一字符串真源**：`include/app_version.h` — `DEF_APP_VER_STRING`（例 **`"0.12"`**）。`video_player.h` 中 **`DEF_VID_VER`** = `"v"` + 该宏，供 UI/日志。
- **Makefile**：`APP_VER` 使用 `major×1024 + minor×16 + micro`；**0.12.0 → 192**。`APP_DESCRIPTION` 形如 **`3ds_v0.12`**（SMDH 长说明）。`APP_VERSION_MAJOR` + `makerom -DAPP_VERSION_MAJOR` 对应 `resource/app.rsf` 的 **RemasterVersion**（存档兼容，产品线仍为 0.x 时不要为「显示 0.12」误改成 12）。
- **已删除**：`cia_high_ram` 目标、`MAKEROM_HIGH_RAM_*`、`RSF_HIGH_RAM_PATH` 及 clean 中对 `*_high_ram.cia` 的引用；只保留常規 `cia_all` / `cia_normal_ram`。
- **远程**：`https://github.com/mockmodular/topos_3ds-video-player` — 当前构建对应字符串 **`v0.12`**；历史附注标签曾含 **`v0.11.0`**（快照 **`d745cdd`**），`main` 在与远程合并后已推送。

## FFmpeg 与 RM / RMVB（未默认可播）

- 按 `library/ffmpeg_build.md` 类 **白名单 configure**：**无 `rm` demuxer**、无 RealVideo/RealAudio 解码器则 **无法** 解 `.rm`/`.rmvb`；文件列表不拦扩展名，缺的是库能力。
- 若要支持：在交叉编译的 `--enable-demuxer=` 中加 **`rm`**；`--enable-decoder=` 中加 **`rv10,rv20,rv30,rv40`** 与常见音轨 **`cook,ra_144,ra_288`**（及个案 **`sipr`** 等），重装 `libavformat.a`/`libavcodec.a` 等到 `library/lib/`，应用侧重链即可。3DS 上 Real 多为软解，高码率易卡。

## 播放器内核精简（vid_state / worker / lifecycle / decode）

- **第一轮**：删仅写入无读者的调试与统计字段（如 **`show_*_graph`**、**`keyframe_list`**、**`packet_buffer_list` / `raw_audio_buffer_list`**、大段 **`raw_video_buffer_list` 历史环**、**`total_frames` / `decoding_*` 累计** 等）；缓冲进度在 **`PLAYER_STATE_BUFFERING`** 时直接读队列深度。
- **第二轮**：删 **`DEBUG_GRAPH_*` 宏与环**、**`video/audio_decoding_time_list`**、**`decoding_avg_*`**、**`Vid_update_decoding_statistics`** 及 **`vid_decode.c`** 中仅为统计服务的 TickCounter；**`previous_ts`** 等仍用于 ~100 ms 的 seek/缓冲逻辑。
- **`menu.c` / `main.c`**：`main` 仅 **`aptMainLoop` → `Menu_main()`**；应用壳与每帧 **`Vid_main`** 在 **`menu.c`**。已删空的 **`Menu_resume` / `Menu_suspend`**；**`Vid_resume`** 不再调用无操作的 **`Menu_suspend`**。
- **`vid_lifecycle.c`**：按需 **`system/draw/draw.h`**，减少对 **`menu.h`** 的依赖；**`vid_draw.c`** 去掉未使用的 **`menu.h`**。
- **Sem worker**：移除空的 **`Sem_worker_callback`** 及对 **`Menu_add/remove_worker_thread_callback`** 的注册；硬件/LCD 等仍在 **`Sem_hw_config_thread`**。
- **Sem 空 API**：删除 **`Sem_resume` / `Sem_suspend` / `Sem_query_running_flag`** 与 **`sem_main_run`**；**`menu.c`** 在 **`Sem_init` 后不再调 `Sem_suspend()`**。
- **MVD**：模式固定 **Unroll4** — 删除 **`mvd_upload_mode`**、**`mvd_stage`**、**`Vid_get/set_mvd_upload_mode`**、**`VID_MVD_UPLOAD_*`**、**DMA staging** 及 **`vid_worker.c`** 中对应 Morton 变体；非 AV1 SBS 走 **`Vid_mvd_sbs_morton_opt`**，2D 走 **`Vid_mvd_2d_morton_opt`**。设置文件 **`<17>`** 仍写 **1** 作兼容位。
- **`video_player.h`**：删除 **`DEF_VID_NAME`** 及已注释名称宏。
- **`vid_draw.c`**：去掉大块已禁用屏上日志的注释噪声（**不改变** **`Util_err` / watch** 行为）。
- **`sem.h` / 错误码**：**`DEF_SEM_ENABLE_UPDATER`** 置 **`false`**，删 **`DEF_SEM_CHECK_UPDATE_URL`**；移除 **`DEF_SEM_ENABLE_SCREEN_RECORDER` 整块**；**`err_types.h` / `err.c`** 删 **`DEF_ERR_CURL_*`** 分支。
- **`cpu_usage`**：**保留** — 数据源给 **`vid_panel_menu.c`** 底屏 CPU 条；**不是** 旧 **`sem.c`** 菜单里的 CPU 图（后者已不在此路径）。

## 外置字体（Exfont）移除 → 系统字

- 删除 **`exfont.c` / `exfont.h` / `exfont_types.h`**；**`menu.c`** 不再 **`Exfont_init` / worker 线程**。
- **`draw.c`**：**`Draw_get_text_size` / `Draw_internal`** 改用 **Citro2D 系统字体**（**`C2D_TextBuf` / `C2D_TextParse` / `C2D_DrawText`** 等）；**`Draw_reinit`** 失败路径释放 **`util_draw_text_buf`**。
- **字号**：**`draw_types.h`** — **`DEF_DRAW_TEXT_SCALE`（0.30f）** 统一所有 **`Draw` / `Draw_c` / `Draw_with_background`**；**`DEF_DRAW_TEXT_C2D_SCALE`** = `DEF_DRAW_TEXT_SCALE * 1.2f`，供 **`C2D_DrawText`** 与 Draw 路径视觉对齐（**`draw.c`** 内对传入 scale 乘 1.2）。
- **顶栏**：**`vid_panel.c`** 路径/标题仍可能用 **`VP_FONT_SCALE` / `C2D_DrawText`**（不经过 Draw），与 Draw 0.30 的换算关系见会话说明。
- **布局**：**`vid_panel_layout.h`** — Lin/Heap **分列锚点**；分辨率与 **SBS/3D** **固定 x** 解耦；多行 **y** 用独立宏；**FPS** 与 **V:** 行 **y** 对齐并用 **`DEF_DRAW_TEXT_C2D_SCALE`**。

## 打开文件、Seek UI、语言包（播放器）

- **连续打开文件**：**`vid_state.h`** 增加 **`play_request_pending`** + 锁；**`panel_open_file`** 只入队 **`DECODE_THREAD_PLAY_REQUEST`** 且 **`message == NULL`**，选项 **`SEND_TO_FRONT | DO_NOT_ADD_IF_EXIST`**；解码线程从 pending 取**最后一次** **`Vid_file*`**，覆盖时 **`free`** 旧的；**`Vid_exit_thread`** 释放残留 pending。
- **Seeking UI**：删除 **`seek_progress`** 等百分比字段与计算；底屏 seek 态只显示 **「Seeking...」** 类英文（**`vid_panel_menu.c`**），不再拼 **`%.2f%%`**。
- **播放器「语言包」机制已删**：**`Vid_msg` 枚举 / `vid_msg[]` / `Vid_load_msg`**、**`romfs/gfx/msg/vid.txt`**、**`Util_load_msg`**（仅播放器消息用）整段移除。**`romfs/gfx/msg/font_name.txt`** 等与 exfont 相关的资源可按需再清。**`sem.c`** 里对 **`bundle.sem_text`** 的 **`Util_parse_file`** 属于 **设置存档格式**，未删。
- **风险备忘（seek 卡 0%）**：**`PREPARE_SEEKING`** 等态会强制 **`seek_progress=0`**；狂拖/狂按若仍卡死，需查 **解码线程状态**、**`decode_thread_notification_queue`** 是否在 handler 里 **错误阻塞**（会吃掉通知）、**`Util_queue_add` 满时阻塞** 等 — 见既有「通知队列不可 blocking 等待」条。

## 其它（第一原理 / 后续可做）

- 会话中列举的 **低风险重构余量**：**`vid_panel.c`** 可按 FILES/PLAYER/SETTINGS 拆文件；**`DEF_SEM_ENABLE_UPDATER` 相关 UI** 在 source 中未再出现则已干净。
- **文档**：**`toposcorenote.md` 与 `.txt`** 若并存建议只留一份；**`note/*.md`** 中关于 **high_ram / mvd_upload / Exfont** 的描述需按上面对照更新。
- **与下文旧章节的关系**：本文件后部「当前架构 / 解耦计划」等仍可能写 **`vid_msg` / `Vid_load_msg` / `Menu_resume`** 等 — 属 **vp3ds 时代抄本**；**以本节与当前源码为准**。
...
- **字符串真源**：`include/app_version.h` — **`DEF_APP_VER_STRING`**（例 **`"0.12"`**）。`include/video_player.h` 中 **`DEF_VID_VER`** = `"v" DEF_APP_VER_STRING`。
- **CIA / NCCH 数值版本**：`Makefile` — **`APP_VER`**，公式 **`major×1024 + minor×16 + micro`**（例 **0.12.0 → 192**）。与 FBI/系统信息里看到的数值一致，**不等于** 上面字符串的逐字符显示。
- **SMDH 长说明**：`Makefile` — **`APP_DESCRIPTION`**（例 **`3ds_v0.12`**）。
- **RemasterVersion**：`resource/app.rsf` 经 **`makerom -DAPP_VERSION_MAJOR`** 代入；**存档兼容**用，勿为 UI 显示把小版本硬塞进「主版本」。
- **约束**：minor 最大 63，micro 最大 15（打包公式限制）。
...
7. **sem 消息索引 / `Util_parse_file` 严格** — 播放器已移除 **`Vid_msg` + `Util_load_msg` + `vid.txt`**；若工程仍用 **`sem_en.txt`** 等，**任意索引缺失**可导致对应菜单块加载失败→**全空白**。维护 **`0~MSG_MAX-1`** 完整覆盖。
...
## Agent 转写同步（最近 15 次会话，按 `agent-transcripts` 的 mtime）

> **来源**：`C:\Users\casts\.cursor\projects\d-3dsvideoplayermock-Topos0-01-topos0-074\agent-transcripts` 下 **最近修改的 15 个** `.jsonl`（2026-04-08～04-12）。  
> **说明**：转写里大量 assistant 段落为 `[REDACTED]`，以下按 **user 诉求 + 可核对仓库的结论** 收敛为架构/位置信息；**不是**逐字聊天记录。  
> **转写 UUID（新→旧）**：`d3768dc4…` `9de77174…` `8c85e3fa…` `0f01d8ac…` `4b9244c1…` `28733b49…` `cc03ef42…` `d894f928…` `ae1e03a5…` `39e3aca7…` `127b628a…` `fcefd98d…` `51e4d7fb…` `5151c104…` `f46a72aa…`

### 与本仓库当前态的差异（重要）

- 转写中曾讨论/声称：**Makefile 改吃 `DEVKITPRO` 的 portlibs + libctru**、删掉 vendor 整树 **`library/include`**、**`stb_image.h`** 放到 **`third_party/include`**、**`copy_devkit_extra_lib.ps1`** 从 `extra_lib` 同步 `.a` 等。  
- **本工作区实测**（同步笔记时）：`Makefile` 仍为 **`wildcard` 收集 `.c`**，`INCLUDES` 仍含 **`library/include`**，**无** `toposre/` 目录，**无** `scripts/copy_devkit_extra_lib.ps1`。以 **`git/磁盘 实际文件`** 为准；下列条目记录「对话中达成的设计与曾改方向」，落地时请再 diff。

### 工程瘦身、白名单与 toposre

- **静态审计结论（死头文件，已删方向）**：`include/system/util/mem_pool.h`、`net_types.h`、`json_types.h` — 零实现或零引用。  
- **冗余 `#include`**：`vid_lifecycle.c`、`vid_draw.c`、`source/system/draw/draw.c` 曾仅含 `cpu_usage.h` 却无调用 → 可删 include；**实际** CPU 采样仍在 `vid_panel_menu.c`、`menu.c`。  
- **`cpu_usage`**：保留 `init` / `exit` / `get_cpu_usage` / `get_core_1_limit`；**移除方向** `query_show_flag` / `set_show_flag` / `draw` 等悬空 API。  
- **`show_color_conversion_graph` 等半废弃**：`vid_state.h`、`vid_lifecycle.c`、`vid_worker.c` 等处注释块 / watch 占位 — 对话要求清理或恢复二选一。  
- **构建**：`Makefile` 用 `wildcard` → 目录内任意 `.c` 都会进 ELF；**白名单**做法 = 显式 `CFILES` 或 `include build/sources.mk`。  
- **链接裁剪**：在安全前提下可试 `-fdata-sections` + `-Wl,--gc-sections`（注意线程入口、回调、弱符号）。  
- **`toposre`**：设计为 **薄壳工程** — `TOPOS_CORE_REL` 指向 `topos.core0.04`，**仅** `build/sources.mk` 列出的 `.c` 参与编译；产物名 **`toposre`**；**`build.bat`** 中 **`gen_banner_wav.py`** 须指向 **`..\topos.core0.04\scripts\`**（因 toposre 无 `scripts`）。  
- **常见故障**：`include $(CURDIR)/build/sources.mk` 报不存在 → 仓库缺 **`toposre/build/sources.mk`** 或 `.gitignore` 忽略了该文件。  
- **「整目录只拷有用文件」到 toposre**：规则 = 编译/链接/romfs/运行所需；不拷 `build/`、`note/`、`toposcorenote.*`、`.git` 等；**`library/lib` 的 `.a`** 若只在本地存在，需自备或从 `%DEVKITPRO%\extra_lib` 同步（对话里提过批量复制与 PowerShell 脚本方案）。

### 音画同步、seek、命令队列（`8c85e3fa…`）

- **同步**：围绕 `video_delay_ms`、音频位置 `audio_current_pos` 与视频位置 `video_current_pos` 的差；用户反馈过符号/观感（声先/画先），要求**去掉临时加上的固定 ms 常数**，改为更本质的算法。  
- **`Vid_cmd_push`**：曾指出 **失败被忽略**、队列 **无锁**；后续方向为失败可记录、上层可感知；**seek** 与 **按键跳秒** 的入队逻辑宜 **合并/对齐**（「写 seek_pos + 入队」+ 可选状态门禁）。  
- **`vid_decode.c` seek**：`PREPARE_SEEKING` 里 `Util_sleep` 轮询重投被评「忙等」→ 方向 **「只保留最后一次 seek 目标」**；`seek_start_pos_after_jump` 初值语义怪 → 建议 **`NAN` 或明确哨兵**；`READ_PACKET_THREAD_FINISHED_SEEKING` 下空循环可读性。  
- **硬解/软解 seek**：用户问两条路径差异及硬解形态（含 unroll 等）。  
- **UI 文案**：「restart play threshold」缩短 → 曾改为 **「buffering」**（用户曾反馈未生效，需以当前源码为准）。  
- **Seek 进度百分比**：曾出现 **瞬间 99.x** 再缓慢涨 — 用户要求用 **代数/状态映射**（a→0、b→0～100、c→100）**避免死写 99**；中间出过 **恒为 0** 的回归需再验。  
- **连按左键黑屏死机**：seek 叠加相关 bug，对话要求修。  
- **连续按左**：后退 n 秒叠加与 seek 队列交互。

### 下屏 UI：`vid_panel.c`（`0f01d8ac…`）

- **标题/路径显示**：限制右边界、与右上按钮留白；后改为 **与按钮脱钩的像素常数**；**文件名** → **硬裁切**（不要 `...` 省略号逻辑）；**路径栏** 短则左对齐、长则 **右对齐裁左**；**标题** 永远左对齐裁右。  
- **统一**：标题与路径的 **最左/最右/字体/字号/y** 应用 **同一套定义**；用户追问多级宏 → 收敛为 **少量钉死常量 + 独立微调量**（如 UI 模式信息与标题/路径 **同为 6px 级但不同宏**）。  
- **Y 键**：从「切换 player / 文件管理」改为 **双方按 Y 均无切换**。  
- **Setting 里「ui 模式」**：除右上角帧率外 **整体右移 3px**；后文还有 **8→6** 等调整。

### 面板文案、SD 目录、播放错误弹窗（`4b9244c1…`）

- **HW color 第二项** 文案 → **「Y2R(auto)」**。  
- **去掉** 在 **3DS 下自动建/检查 `Video_player` 文件夹** 的逻辑。  
- **无法打开格式** 的弹窗：文字与 **OK** 在弹窗内 **水平居中**；要求定义 **解耦**（每行文字中心、按钮中心对齐弹窗内容区中心）。

### 导航、Lawvere、HW、机型识别、CPU 条（`28733b49…`）

- **右下角**：player 显示 **「file(x)」** — 切换 player ↔ 文件管理；**Y** 也可触发。  
- **右上角**：player 与文件管理均为 **Setting**；Setting 右上角 **toggle** 回到进入 Setting 前的界面。  
- **Setting 右下角**：**info**（及 **Y**）→ **Lawvere** 界面；**player 无 Lawvere 入口**。  
- **Lawvere 文案** 曾讨论数学家梗；**info 图层** 须盖在 Setting 之上。  
- **HW decode 持久化**；**不因 old3ds 模式强行改 sw** — 逻辑为 **能硬则硬**；文案 **「HW」→「HW(auto)」**，**「Y2R*2」→「Y2R X2(auto)」**。  
- **fakemodel / 机型**：用户理清 CFG、型号与核心数覆盖关系；最终方向：**一律用 CPU 核心数** — **4 核 → N3DS，否则 → O3DS**（对话里对 **off** 与 **假 O3DS** 是否尝试硬解有细分：如 **off + 真 O3DS → 永久软解** 等，以最终实现为准）。  
- **CPU 占用条**：**2 核可用** 时只显示 **C0 C1**，否则 **C0～C3**；**在 UI 模式初始化时判一次**，非每帧（若 UI 模式开关会重建 UI 则再判）。

### 持久化与目录清理（`cc03ef42…`）

- **统一设置文件**：`topos_setting.txt`（曾议 `.md` 后改 `.txt`）；**路径** → **`/3ds/topos/topos_setting.txt`**；**无迁移**、无旧文件名转发 — **没有就新建默认**。  
- **源码**：`topos_setting_md.c` 重命名为 **`topos_setting.c`** 方向。  
- **删除**：错误导出、**logs/**、**ver/**、**screen_recording/** 等机制与代码；**saved_pos** 类记忆播放迁移到 **`topos` 下** 再 **整功能删除**（含 panel 选项）。

### 解码与发布（`d894f928…` `ae1e03a5…` 等）

- **现象**：H.264 **硬解播放到结尾** 后 **拖动进度条** 可能 **变成软解** — 对话讨论原因与是否在播放结束 **清空选择**。  
- **GitHub**：`mockmodular/topos_3ds-video-player` — 上传源码、**elf/3dsx/cia**、部分 bat/mkv、**Release** 操作、新建仓库流程等在对话中出现（非代码架构，记作 **发布位置**）。

### 小改动与其它（`5151c104…` `f46a72aa…`）

- **uimod** 选项文案 → **off | on**，**off 左 on 右**，默认 **on**。  
- **tex filter** Setting 默认值是否为 **auto**（对话提问，需对照 `vid_panel.c`）。  
- **空 path 点播放**：删除弹窗，**无操作**；**保留**「无法打开文件/类型」的 **FILES 专用小窗**；曾讨论与 **通用错误 UI** 文案统一为 **同一套小窗文案**。  
- **d3768dc4…**：说明 **Agent 转写路径** 与 **无法跨实例读全聊天记录**（元信息）。

---

## Agent 转写同步（第 16～20 个会话，按 mtime）

> **选取**：同上目录，在「最近 15 个」之后顺延 **5 个** `.jsonl`（约 **2026-04-08**）。  
> **UUID（新→旧）**：`50da3c1d…` `6024bcb9…` `2f1c7fd0…` `23a0a37f…` `b3935607…`

### `50da3c1d…`（元检索）

- 在转写目录内搜索 **「弹窗」** 相关会话的索引型对话；归纳命中主题包括：文件管理打不开文件不弹窗（`6024bcb9…`）、Lawvere 触摸关闭（`b3935607…`）、未选文件播放弹窗删除（`f46a72aa…`）、设置/音轨弹窗残留（`fe122522…`）、Banner 工具弹窗（`1f76ac0d…`）、文件错误 overlay 置顶（`3f88c12d…`）等。**本段无独立功能改动**，作交叉引用用。

### `6024bcb9…`（设置大改 + 导航 + 持久化 + 文件报错）

- **根菜单顺序**：`volume` → `custom` → `video` → `advance`（后文有 **Advanced** 拼写统一、**全小写** 要求：`volume` / `custom` / `video` / `advanced` 等，专有缩写如 LR 可大写）。  
- **Video 子菜单**（由原 LCD/屏幕模式迁出）：  
  - **「3d or 2d」** 三档：`3d` / `2d` / **`auto`**（下屏 chip 文案简写）；**auto** 用 **`video_info[0].width` / `height`（显示/解码尺寸，非 AV1 的 128 padding）** 判定 SBS：`(h==240 && w>=640) || (w==800 && h<=240)`。涉及 `VP_SUBPAGE_VIDEO`、`VP_SETTING_ID_ENTER_VIDEO`、`screen_mode_ui_from_sem` / `screen_mode_sem_from_ui`，**存档数值兼容**。  
  - **tex filter** 三档：英文化（Bilinear / Nearest / Auto），auto 分支按 **2D / 3D（含 auto 判出的 2D）** 与 **w/h 条件** 选 NEAREST vs LINEAR（细则见转写）。  
  - **缩放**：「点对点」仅 **2D** 且特定偶数分辨率时 **上屏居中 1:1**，且 **强制 NEAREST 覆盖 tex filter**；「等比」为默认全屏逻辑。  
  - **3d eyes swap**：从 Switch setting 迁入，`L<>R` / `normal`。  
- **Custom 子菜单**：`seekstep`（默认 10）、`remember pos`、`autodim`、`eco mode`；后增 **根目录**（`movie` 文件夹兜底 TF 根）、**ui mod**（off 时隐藏下屏信息 UI 但保留进度条/时间/seek；后补丁：**左上文件名不受 ui mod 隐藏**）。子菜单文案曾要求 **全英文**。  
- **Advanced 子菜单**：迁入 `hw decode`、`hw color`、`fakemodel`、`multi decode`、`no audio`、`no video`；**mvdupload** 从设置删除、**写死 unroll4**。  
- **默认值**：video 里 3d2d **auto**、tex filter **auto**、缩放 **等比**、eyes **normal**；**volume 100**。  
- **分辨率字段约定**：Video 子菜单内所有「宽高」逻辑用 **`width`/`height`（Video width/height）** 以便 **AV1** 正确（不用 `codec_*` 的 128 对齐假分辨率）。  
- **下屏 UI**：`sbs` / `3d` 标移到与 **分辨率、帧率** 同一排并对齐；**2 核机** 只显示两路 CPU 条、**4 核** 四路（与后文 `28733b49…` 会话可对照）。  
- **持久化**：询问是否全部子项持久化；**sem 相关** 是否迁入 `vid_settings.txt` 与同机制；**3d eyes swap** 持久化；删掉 **`Vid_exit_thread` 末尾多余 `Vid_save_settings()`**；音量条用 **方向键** 调是否触发保存。  
- **三界面导航（旧版描述，后续会话有改版）**：文件管理 **Start** 进 Setting、Player/文件管理 Start 进 Setting；Setting **Start** 或右下 **toggle(s)** 回到进入前界面；Setting 左下 **back** 只回到设置根、不退出 Setting；各角键位文案 **folder(x)** / **play(a)** / **info(y)** 等；后补：**下屏三键文字居中**。  
- **布局**：进度条、左侧时间、右侧 seeking **整体上移 8px**。  
- **Setting 光标**：切换面板再回来 **不重置** 到第一项。  
- **Bug 报告**：文件管理打开 **无法打开的文件** **不再弹窗报错** — 与 `vid_decode.c` 里 `error:` 分支在 **`switch_player_panel_from_files_open`** 时不走 `Util_err_set_show_flag`、`Util_decoder_open_file` 失败未设文案等有关（后续会话继续修 FILES 专用 overlay）。

### `2f1c7fd0…`（tex filter 机制说明）

- **`vid_panel.h` 无 texfilter 声明**；项在 **`vid_panel_settings.c`**（原在 Switch 子页的 **Tex filter** Toggle）。  
- **`vid_player.use_linear_texture_filter`**：改动后对 **所有缓冲 × 眼** 调 `Vid_large_texture_set_filter(..., Vid_effective_use_linear_texture_filter(k))`。  
- **`Vid_effective_use_linear_texture_filter`（`vid_worker.c`）**：用户开 LINEAR 仍可能被 **`Vid_texture_dimensions_prefer_nearest(codec_w, codec_h)`**（`vid_texture.c`）打回 NEAREST（如 **h==240 且宽偶数**、**w==400 且高偶数** 等）。  
- **GPU**：`Vid_large_texture_set_filter` → `Draw_set_texture_filter`。持久化在 **`vid_settings.c`**，默认曾 **true**。`MSG_TEX_FILTER` 可能为遗留枚举无绑定。

### `23a0a37f…`（分辨率字段与 AV1）

- **`menu.h` 不存分辨率**；用 **`Media_v_info`**（`media_types.h`）经 **`vid_player.video_info[]`**。  
- **`width` / `height`**：解码器报告的画面尺寸；**`codec_width` / `codec_height`**：按块 **对齐** 后的缓冲尺寸（**AV1 用 128**，其它 **16**，见 **`decoder_video_soft.c` `DecoderVideoSoft_get_info`**）。  
- **菜单显示**：AV1 为避免把 padding 当分辨率，**优先显示 `width`/`height`**（**`vid_panel_menu.c`** 片段）。**`is_av1_codec`** 标记专用路径，非分辨率字段。

### `b3935607…`（Lawvere / quick overlay）

- **Player 按 Y** 的 **Lawvere** 层：**手指按下** 且触点落在与 **`Vid_panel_draw_overlay`** 一致矩形内 → **关闭**（`s_quick_menu_open = false`），在 **`Vid_panel_list_press`** 的 **`VID_PANEL_PLAYER`** 分支里、进度条等逻辑 **之前** 处理。  
- 几何与 **`VP_OVERLAY_MARGIN`**、**`VP_OVERLAY_PANEL_H`** 等一致；可抽 **`vp_quick_overlay_hit()`** 与绘制共用。弹窗外区域行为按实现版本为准。
